...
namespace Executing.Infrastructure.Persistance.Repositories
{
    public class CardRepository : ICardRepository
    {
        ...
        {
           return await _context.Cards.Include(x => x.Labels).Include(x => x.Tasks).Include(x => x.Tasks).Where(x => x.Id == id).FirstOrDefaultAsync();
        }
        ...
        public async Task<int> GetCardsCountByColumn(Guid? columnId)
        {
            return await _context.Cards.Where(x => x.ColumnId == columnId).CountAsync();
        }
